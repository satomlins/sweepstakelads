...
- `_resolve_transclusions(pages)` runs after `fetch_all_wikitext()` and before parsing; it finds `{{#lst:Page|section}}` labeled section transclusions in the fetched wikitext, batch-fetches any target pages not already in the set, extracts content between `<section begin=X/>` / `<section end=X/>` markers (the section name may be bare or double-quoted — both forms appear on Wikipedia), and replaces the transclusion tags with the resolved content. Falls back to leaving the tag in place (silently skipped by the parser) on any failure. This handles Wikipedia editors moving match details to dedicated articles - common for notable matches as the tournament progresses (e.g. the entire Round of 32 lives on its own page `2026 FIFA World Cup round of 32` and is transcluded into the main knockout page).
- `fetch_all_matches()` dedupes by `(date, home_team, away_team, stage)` after parsing. Reason: the knockout page transcludes the final from `FINAL_PAGE` *and* we still scrape `FINAL_PAGE` directly as a fallback, so the final would otherwise appear twice. The dedupe is a no-op when the transclusion is absent.
- `fetch_all()` is the bracket-aware variant — returns `(matches, pages)` where `pages` is the post-transclusion `{title: wikitext}` dict. `tournament.refresh()` uses this to feed the knockout wikitext into `bracket.resolve_placeholders` for placeholder substitution. `fetch_all_matches()` is preserved as a thin wrapper for callers that only need the match list.
...
- Future knockout rounds appear with placeholder team names (e.g. "Winner Match 75", "Winner Group A", "3rd Group A/B/C/D/F") — the scraper passes these through as-is after stripping wikitext markup. `bracket.resolve_placeholders` (called from `tournament.refresh`) substitutes the first two kinds; third-placer slots are left for Wikipedia editors.
- Uses `logging` (module-level `logger`); partial failures logged as `logger.warning`

**`bracket.py`** — placeholder resolver for knockout fixtures:
- `resolve_placeholders(matches, group_standings, knockout_wikitext)` returns a NEW matches list with team-name placeholders substituted.
- Two kinds of substitution:
  1. **Group placeholders** — `"Winner Group X"` / `"Runner-up Group X"` → top-of-group / second-of-group from `compute_group_standings`. Only applied when all 6 matches in the group are finished (every row has `PL == 3`).
  2. **Knockout cascade** — `"Winner Match N"` / `"Loser Match N"` → winner / loser of FIFA match number N. Outcomes are read from the bracket section of the knockout page (`<section begin="Bracket"/>` … `{{#invoke:RoundN|N32}}` block), which Wikipedia editors keep up to date with score cells per bracket entry. Substitution loops to fixed point so an R32 result propagates through R16 → QF → SF → Final in a single call.
- `parse_bracket_section(wikitext)` extracts the bracket entries in bracket order (16 R32, 8 R16, 4 QF, 2 SF, Final, third-place) with decoded team names (FIFA 3-letter code → display name) and integer scores. It splits round groups by HTML round-marker comments (`<!--Round of 32-->`, etc.).
- `_derive_feeder_match_numbers(entries)` assigns FIFA match numbers to each feeder position using standard N32 pairing — round R position k is fed by round (R-1) positions (2k-1) and (2k), with Final/third-place both consuming SF positions 1 and 2. Match numbers are read out of `"Winner Match N"` / `"Loser Match N"` references in the *later* round's entries (a finished match no longer has its own `Match N` label in its score field — the later round is the only source).
- **3rd-placer slots like `"3rd Group A/B/C/D/F"` are intentionally left untouched.** The FIFA third-placer allocation rule (which third-placed group gets which R32 slot) is too involved to reimplement; we rely on Wikipedia editors to fill those slots once group-stage standings settle.
...
- **In/Out (elimination)**: `_team_out_status(team, group_standings, matches)` is the source of truth for the `In` column. A team is `Out` only when it has played 3 group matches AND is either bottom of its group (last row of the H2H-ranked standings, group has 4 teams) or in the bottom 4 of the third-place table. Otherwise `In`. Knockout-stage results deliberately do not affect In/Out — only group-stage standing does, so a knockout loser stays `In` on the leaderboard. `compute_team_table` calls `compute_group_standings` once and feeds it into `_team_out_status` for every team.
...
- `refresh()` calls `scraper.fetch_all()` → `compute_group_standings` → `bracket.resolve_placeholders` (substitutes "Winner Group X", "Runner-up Group X", "Winner/Loser Match N") → `compute_team_table` / `compute_person_table` → writes cache using **atomic temp-file writes** (`_atomic_write_csv`, `_atomic_write_json`, `_atomic_write_text`); `last_updated.txt` written last so a partial write shows as stale rather than corrupt; fixtures sorted ascending by `DatetimeUTC` before caching. Resolved (not raw) match names are what gets persisted into `fixtures.csv` and rendered in the UI.
