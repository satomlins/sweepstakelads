...
- **Flag-emoji toggle**: footer button (`flags-toggle`) flips a `dcc.Store(id="show-flags")` between `False` (names) and `True` (flags). `update_all` reads the store and calls `_apply_flags(df, cols, show)` on the Team Table, each Group Stages mini-table, the Third-Place table, and the four fixture output DataFrames (`recent_out`, `upcoming_out`, `all_results_out`, `all_upcoming_out`) for the `Home` and `Away` columns. The substitution falls back to the original name when a team is not in the `FLAGS` dict, so knockout placeholders like `Winner of Match 57` and unmapped finalists render as text. Substitution runs **after** `_add_owner_cols`, **after** the owner filter, and **after** the third-place table's merge with the draw — all three key on the original team names. The toggle is a pure swap — cells show the name *or* the flag, never both. State does not persist across reloads.
- **Mobile card layout**: on ≤480px viewports, all four fixture/result tables (`recent-table`, `upcoming-table`, `all-results-table`, `all-upcoming-table`) switch from `DataTable` to a card-based layout. Both layouts are rendered server-side; CSS toggles visibility (`desktop-only`/`mobile-only` classes). Cards are grouped by local date with a date header; each card has three lines: (1) Stage·Time dimmed meta, (2) matchup line, (3) owner names. The matchup and owner lines both use a **three-column flex layout** — home right-aligned | separator centred | away left-aligned — so the score/`v`/`·` is always the visual midpoint. Team names in the matchup line are coloured by their owner's colour (same as `COLOURS` dict); winner is bolded via `.winner` span. Results scores have spaces around the en-dash (`"2–1"` → `"2 – 1"`, first occurrence only so `(pens 4–3)` is unchanged). `_fixture_cards(df, is_result)` and `_owner_span(name)` helpers in `app.py` build the card tree; `update_all` has four new `Output("*-cards", "children")` targets (`recent-cards`, `upcoming-cards`, `all-results-cards`, `all-upcoming-cards`) populated alongside the existing DataTable outputs.
...
- Mobile card layout classes: `.mobile-only`/`.desktop-only` toggle at 480px; `.mobile-cards` wrapper; `.card-date-header`; `.match-card`; `.card-meta` (centred); `.card-matchup` (flex row) with `.card-matchup-home` (flex:1, right-align), `.card-matchup-score` (fixed centre), `.card-matchup-away` (flex:1, left-align); `.card-matchup .winner` (bold); `.card-owners` (flex row) with `.card-owner-home`, `.card-owner-sep`, `.card-owner-away`
