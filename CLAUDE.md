...
- **Mobile card layout**: on ≤480px viewports, all four fixture/result tables (`recent-table`, `upcoming-table`, `all-results-table`, `all-upcoming-table`) switch from `DataTable` to a card-based layout. Both layouts are rendered server-side; CSS toggles visibility (`desktop-only`/`mobile-only` classes). Cards are grouped by local date with a date header; each card shows Stage·Time (dimmed), team matchup (winner bolded via `.winner` span), and owner names in their colours. `_fixture_cards(df, is_result)` in `app.py` builds the card tree; `update_all` has four new `Output("*-cards", "children")` targets populated alongside the existing DataTable outputs.
